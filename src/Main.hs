...
altmain = do
    parent <- elm_win_util_standard_add (fromString "mainwnd") (fromString "genProxy")
    evas_object_smart_callback_add parent (fromString "delete,request") on_done nullPtr
    
    ent <- elm_entry_add parent
    elm_entry_scrollable_set ent True
    elm_object_text_set ent (fromString "this is a single line")
    evas_object_size_hint_weight_set ent c'EVAS_HINT_EXPAND 0.0
    evas_object_size_hint_align_set ent c'EVAS_HINT_FILL 0.5
    elm_entry_single_line_set ent True
    elm_win_resize_object_add parent ent
    evas_object_show ent
    
    evas_object_show parent

main = withElementary $ do
